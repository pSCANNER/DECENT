...
public class OysterAttributes {
    ...
    private static Map<String, OysterRule> identityRulesMap = null;
...
//            logger.addHandler(consoleHandler);
    ...
    /**
     * Uses the new (as of 9/8/2015) Java 8 stream interface.
     * 
     * @return Return a map of keys vs. rules using a first use of Java 8 lambdas and streams.
     * 
     */
	public static Map<String, OysterRule> getIdentityRulesMap() {
		if (identityRulesMap == null) {
			
			identityRulesMap = identityRules.stream().collect(Collectors.toMap(OysterRule::getRuleIdentifer, item -> item));
			OysterRule generalMatchRule = new OysterRule();
			generalMatchRule.setRuleIdentifer("@");
			generalMatchRule.setRuleDescription("Unknown Rule Match");
			identityRulesMap.put(generalMatchRule.getRuleIdentifer(), generalMatchRule);
		}
		return identityRulesMap;
	}
}
